                ...
                case ETIE_PRESSED_DOWN: {
                    ...
                    if (TouchID == -1) {
                        ...
                        if (Device) {
                            ...
                            IGUIElement *logo = Device->getGUIEnvironment()->getRootGUIElement()->getElementFromId(
                                    GUI_IRR_LOGO);
...
//	param.DriverType = EDT_OGLES1;				// android:glEsVersion in AndroidManifest.xml should be "0x00010000" (requesting 0x00020000 will also guarantee that ES1 works)
    ...
    param.WindowSize = dimension2d<u32>(640 * 3, 320 *
                                                 3);    // using 0,0 it will automatically set it to the maximal size
...
//	ANativeActivity_setWindowFlags(app->activity, AWINDOW_FLAG_FULLSCREEN, 0);
    ...
    */
    ...
    __android_log_print(ANDROID_LOG_ERROR, "test", "Window size:(%d/%d)\nDisplay size:(%d/%d)",
                        windowWidth, windowHeight, displayMetrics.widthPixels,
    ...
    IGUIStaticText *text = guienv->addStaticText(stringw(displayMetrics.xdpi).c_str(),
                                                 rect<s32>(5, 5, 635, 35), false, false, 0,
                                                 GUI_INFO_FPS);
    ...
    if (logo && logo->getRelativePosition().getWidth() < minLogoWidth) {
        ...
        */
        ...
        logoPos.LowerRightCorner.Y =
                logoPos.UpperLeftCorner.Y + (s32) ((f32) logoPos.getHeight() * scale);
